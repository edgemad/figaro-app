# Figaro - Private local AI assistant

**Figaro** is a free, standalone desktop AI assistant that runs 100% offline on your computer. It is a community-branded build of [Jan](https://github.com/janhq/jan) (Apache-2.0) — same engine, rebranded identity (name, icon, window title), with automatic file saves removed: everything Figaro creates (images, Word documents, notes) appears as a preview/download link in your chat and is never written to your disk without you choosing to save it.

## Download installers

Get the latest installer for your platform from the [Releases](https://github.com/edgemad/figaro-app/releases) page:

| Platform | Format |
| --- | --- |
| macOS (Apple Silicon + Intel) | `.dmg`, `.zip` |
| Windows (x64) | `.exe` (NSIS setup) |
| Linux (x64) | `.deb`, `.AppImage` |

> **Note:** These are community, **unsigned** builds. On macOS you'll need to right-click → Open (or `xattr -dr com.apple.quarantine`) the first time; on Windows/SmartScreen choose "More info → Run anyway".

## This repository

This is the upstream [janhq/jan](https://github.com/janhq/jan) source at **v0.8.4**, modified only by the `figaro` branding and build workflow:

- `src-tauri/tauri.conf.json` — product name **Figaro**, identifier `com.figaro.app`, updater **disabled** (this build never phones home).
- Window title / sidebar / default assistant renamed to Figaro.
- `src-tauri/icons/` — black-cat-on-orange-tile icon (Twemoji cat, [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/)).
- `.github/workflows/figaro-build.yml` — builds macOS (dmg+zip), Linux (deb+AppImage) and Windows (exe) installers on every `v*` tag.

## Build from source

Prereqs (see upstream docs): Node ≥ 20, Yarn 4.5.3, Rust, Make.

```bash
git clone https://github.com/edgemad/figaro-app
cd figaro-app
make dev        # development
make build      # production installer for your current platform
```

## License

Apache-2.0 — see [LICENSE](LICENSE). Based on [Jan](https://github.com/janhq/jan) by Menlo Research Pte. Ltd. App icon derived from [Twemoji](https://github.com/jdecked/twemoji) (black cat) © 2020 Twitter, Inc. / licensed [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/).
